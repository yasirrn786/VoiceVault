...
def check_quality(samples: np.ndarray, sample_rate: int = 16000) -> AudioQuality:
    ...
    issues: list[str] = []
    if duration < .45: issues.append("very_short_chunk")
    if rms < .004: issues.append("silence" if rms < .001 else "low_rms")
    if clipping > .02: issues.append("clipping")
...
def pcm_to_wav(audio: bytes, sample_rate: int = 16000) -> bytes:
    ...
    with wave.open(output, "wb") as wav:
        wav.setnchannels(1); wav.setsampwidth(2); wav.setframerate(sample_rate); wav.writeframes(audio)
